...
contract Avtars is Ownable, ERC721Enumerable {
    ...
    string public baseURI =
        "ipfs://QmZLfZHMA5bXDPWRAeMvBAGokxgm1rF2DbgyrFrTfeoAv4/";
    ...
    event directReferralRewardTransferred(
        address user,
        address directReferrer,
        uint256 rewardAmount
    );
    event indirectReferralRewardTransferred(
        address user,
    ...
    function mintNFT(
        ...
        require(
            ...
            "You already have an NFT of this level!"
        );
        require(!paused, "Minting is paused");
        require(
            totalSupplyOfLevel(_level) < NFT_Quantities[level],
            "Cannot mint more NFTs of this level"
        );
        ...
        if (msg.sender != owner() && !whitelisted[msg.sender]) {
            ...
            if (_delegate) {
                ...
                if (indirectReferrer != address(0)) {
                    ...
                    emit indirectReferralRewardTransferred(
                        msg.sender,
                        indirectReferrer,
                        indirectReferralReward
                    );
    ...
    function getRankUplifting(
        ...
        for (uint256 i = 0; i < referrals[_user].length; i++) {
            ...
            if (
                setUserRank[member]._rankChanged == true &&
                setUserRank[member]._rank > nftlevel
            ) {
                memberrankCount++;
                staisfied = true;
    ...
    function getUserRankBonus(address _user)
    public
    view
    returns(AddressRank[] memory) {
    ...
    returns(address[] memory) {
        ...
        uint256 arrayLength = teamAddresses.length > 0 ?
    ...
    function teamSalesInformation(address _user)
    public
    view
    returns(teamStatistic[] memory) {
        address[] memory _referrals = referrals[_user];
        teamStatistic[] memory teamStatisticsArray = new teamStatistic[](
            _referrals.length
        );
        for (uint256 i = 0; i < _referrals.length; i++) {
            ...
            teamStatistic memory teamStatisticsInfo = teamStatistic(
                user,
                userRank,
                Totalpartner,
                ownNFT,
    ...
    returns(address[] memory) {
        ...
        address[] memory teamAddressesWithoutFirst = new address[](
    ...
    function getAdminRankChanged(address user, uint256 _rank)
    internal
    view
    returns(bool) {
        address[] memory _referrals = referrals[user];
        ...
        for (uint256 i = 0; i < _referrals.length; i++) {
            ...
            if (
                setUserRank[member]._rankChanged == true &&
                setUserRank[member]._rank > _rank
            ) {
                return true;
            }
            if (referrals[member].length > 0) {
    ...
    returns(address[] memory) {
        ...
        for (uint256 i = 0; i < numReferrals; i++) {
            ...
            address[] memory concatenated = new address[](
                teamAddresses.length + memberTeam.length
            );
    ...
    function legSearch(
        ...
        for (uint256 i = 0; i < referrals[member].length; i++) {
            ...
            if (
                setUserRank[referrer]._rankChanged == true &&
                setUserRank[referrer]._rank > nftlevel
            ) {
                return true;
    ...
    function setReferrer(address referrer) internal {
        if (myReferrer[msg.sender] == address(0)) {
            ...
            require(
                myReferrer[msg.sender] == referrer,
                "fill correct reffral address"
            );
        }
    }

    function sortAddressesByTimestamp(
        address[] memory addressesArr,
        uint256[] memory timestampsArr
    ) internal pure {
        uint256 n = addressesArr.length;
        for (uint256 i = 0; i < n - 1; i++) {
            for (uint256 j = i + 1; j < n; j++) {
                if (timestampsArr[i] > timestampsArr[j]) {
                    (addressesArr[i], addressesArr[j]) = (
                        addressesArr[j],
                        addressesArr[i]
                    );
                    (timestampsArr[i], timestampsArr[j]) = (
                        timestampsArr[j],
                        timestampsArr[i]
                    );
                }
    ...
    function setPancakeRouterAdress(address _newpancakeRouterAdress)
    public
    onlyOwner {
    ...
    function createReferralArray(address _ref, address[] memory to)
    public
    onlyOwner {
